...
public class FuturesLibrary {
    ...
    public static int countMatchingBooks(Library library,
                                         ...
                                         int countBooks) {
        int count = 0;
        if (countBooks<0 || startBookIndex < 0 || startBookIndex + countBooks > library.getNumberOfBooks() || startBookIndex >= library.getNumberOfBooks()) {
    ...
    public static int countMatchingBooksWithThreads(Library library,
        ...
        Future<Integer>[] futures = new Future[countThreads];
        int batch_size = library.getNumberOfBooks() / countThreads;
        if (library.getNumberOfBooks() % countThreads == 0) {
            batch_size = library.getNumberOfBooks() / countThreads;
        } else {
            batch_size = library.getNumberOfBooks() / countThreads + 1;
        }
        for (int i = 0; i < countThreads; i++) {
            final int start_index = i * batch_size;
            final int size = (start_index + batch_size < library.getNumberOfBooks()) ? batch_size : library.getNumberOfBooks() - start_index;
            futures[i] = executor.submit(() -> countMatchingBooks(library, predicate, start_index, size));
        }
